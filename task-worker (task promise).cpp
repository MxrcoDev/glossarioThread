...
public:
    ...
    void completaTask(const string& result) {
    ...
    void ElaboraTask() {
        ...
        while (true) {
            ...
            if (haTask) {
                ...
                res = "La task " + to_string(task.id) + " e' stata completata dal worker #" + to_string(id) + "\n";
                task.completaTask(res);
...
private:
    ...
    vector<future<string>> futures;         // Vettore per raccogliere i future delle task
...
public:
    ...
    void generaTasks() {
        for (int i = 0; i < N_TASK; i++) {
            lock_guard<mutex> lock(memoryMutex);
            Task task(i); // Creo la Task
            futures.push_back(task.getFuture());
            codaTask.push(move(task)); // Aggiungi la task alla coda
    ...
    void generaWorkers() {
        for (int i = 0; i < N_WORKERS; i++) {
            auto worker = make_unique<Worker>(codaTask, i, stopFlag);   // Creo un worker
    ...
    // Restituisce i risultati delle task tramite i futures
    void restituisciRisultati() {
        cout << "\n========== RISULTATI ==========\n";
        for (auto& fut : futures) {
            cout << fut.get() << endl;  // Stampa il risultato della task
        }
        cout << "==============================\n";
    }
};
